...
(*https://stackoverflow.com/questions/30830179/error-reference-to-undefined-global-num*)
...
let draw_pixel (x:int) (y:int) (c:char) =
    Printf.printf "\027[%d;%dH%c" x y c;;
let loop (c:char) = 
    cls(); 
    Printf.printf "(%c)" c;;
...
let tab = Array.make_matrix 1000 1000 0.0;;
let rec lo (f) (p) (x:int) = 
    if (p x) then x else (lo f p (f x));;
let p = perlin_init permutation in lo (fun (i) -> tab.(2).(0)<-(*int_of_float (perlin_noise p 3.14 42.0 7.0)*)2.0; (i-1)) (fun i -> i < 9) (0);;
tab.(0).(0) <- (let p = perlin_init permutation in perlin_noise p 3.14 42.0 7.0);;
let p = perlin_init permutation;;
lo (fun (i) -> tab.(2).(0)<-perlin_noise p 3.14 42.0 7.0; (i-1)) (fun i -> i < 9) (0);;
let rec mlkqjsdf i =
    if i < 1000 then
        ((tab.(0).(i) <- (let p = perlin_init permutation in perlin_noise p (3.14+.(float_of_int i)) 42.0 7.0)); mlkqjsdf (i+1))
        else 0;;
mlkqjsdf 0;;
tab;;
let p = perlin_init permutation in perlin_noise p 3.14 42.0 7.0;;
let p = perlin_init permutation in perlin_noise p 3.14 41.0 7.0;;
let get_termainal_size () =
    Printf.printf "\027[s\027[999;999H\027[6n\027[u";;
get_termainal_size ();;
let table n =
    for row = 1 to n do
      for column = 1 to n do
        print_float( tab.(row).(column));
        print_string " "
      done;
      print_newline ()
    done;;
...
let syscall cmd =
    ...
    (Buffer.contents buf);;
let listing = syscall "tput cols" ;;
let listing = syscall "tput lines" ;;
let main () = 
    (*draw_pixel 10 20 'e';*)
    let c = get1char() in loop c;
    print_endline "";;
let rec whileTrue() = 
    main();
    whileTrue();;
