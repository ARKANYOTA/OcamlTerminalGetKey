let smoothstep (w:float): float = 
  if (w <= 0.0) then 0.0 else
  if (w >= 1.0) then 1.0 else
  w *. w *. (3.0 -. 2.0 *. w);;

let interpolate (a0:float) (a1:float) (w:float):float =
  a0 +. (a1 -. a0) *. smoothstep w ;;

class gradient x y (z: int*int) = 0
  ;;

let dotGridGradient (ix:int) (iy:int) (x: float) (y:float) =

  (*extern float Gradient[IYMAX][IXMAX][2] in*)

  let dx = x -. float_of_int ix in
  let dy = y -. float_of_int iy in

  (dx*Gradient[iy][ix][0] + dy*Gradient[iy][ix][1]);;

let perlin (x:float) (y:float) = 
  let x0 = int_of_float x;
  let x1 = x0 + 1;
  let y0 = int_of_floor y;
  let y1 = y0 + 1;

  let sx = x - (float)x0;
  let sy = y - (float)y0;

  let n0 = dotGridGradient(x0, y0, x, y) in
  let n1 = dotGridGradient(x1, y0, x, y) in
  let ix0 = interpolate(n0, n1, sx) in
  let n0 = dotGridGradient(x0, y1, x, y) in
  let n1 = dotGridGradient(x1, y1, x, y) in
  let ix1 = interpolate(n0, n1, sx) in
  let value = interpolate(ix0, ix1, sy) in
